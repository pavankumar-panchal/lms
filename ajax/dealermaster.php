	...
	case "save":
		...
		if ($form_recid == "") {
			...
			if ($message == "") {
				...
				// First query: UPDATE lms_users
				$query1 = "UPDATE lms_users SET username = ?, password = ?, disablelogin = ?, relyonexecutive = ? WHERE referenceid = ? AND type = 'Dealer'";
				$result = runmysqlquery($query1, 'sssss', $form_username, $form_password, $form_disablelogin, $form_relyonexecutive, $form_recid);

				// Second query: SELECT regions (fetching related data)
				$query2 = "SELECT * FROM regions WHERE distcode = ?";
				$result1 = runmysqlqueryfetch($query2, 's', $form_district);

				// Third query: UPDATE dealers (using values fetched from the second query)
				$query3 = "UPDATE dealers SET stateid = ?, districtid = ?, state = ?, district = ?, dlrname = ?, dlrcompanyname = ?, dlraddress = ?, dlrcell = ?, dlrphone = ?, dlremail = ?, dlrwebsite = ?, managerid = ?, branch = ?, showmcacompanies = ? WHERE id = ?";
				$result = runmysqlquery(
					$query3,
					'ssssssssssssssi',
					$form_state,
					$form_district,
