...
switch ($submittype) {
	...
	case "save":
		...
		if ($form_recid == "") {
			if ($message == "") {
				...
				$query = "SELECT * FROM lms_users WHERE username = ?";
				$result = runmysqlquery($query, 's', $form_username);
			...
			if ($message == "") {
				...
				$query = "SELECT * FROM lms_subadmins WHERE sadname = ?";
				$result = runmysqlquery($query, 's', $form_name);
			...
			if ($message == "") {
				...
				// Insert into `lms_subadmins`
				$query = "INSERT INTO `lms_subadmins` (sadname, sademailid, transferuploadedleads, cell, showmcacompanies) 
          VALUES (?, ?, ?, ?, ?)";
				runmysqlquery($query, 'sssss', $form_name, $form_email, $transferuploadedleads, $cell, $showmcacompanies);

				// Retrieve the ID of the newly inserted subadmin
				$query = "SELECT id FROM lms_subadmins WHERE sadname = ?";
				$result = runmysqlqueryfetch($query, 's', $form_name);
				$subadmin_id = $result['id'];

				// Insert into `lms_users`
				$query = "INSERT INTO `lms_users` (username, password, type, referenceid, lastlogindate, logincount, disablelogin) 
          VALUES (?, ?, 'Sub Admin', ?, '', '0', ?)";
				runmysqlquery($query, 'sssi', $form_username, $form_password, $subadmin_id, $disablelogin);

				// Insert into `lms_logs_event`
				$query = "INSERT INTO lms_logs_event (userid, system, eventtype, eventdatetime) 
          VALUES (?, ?, '6', ?)";
				runmysqlquery($query, 'sss', $userslno, $_SERVER['REMOTE_ADDR'], datetimelocal("Y-m-d H:i:s"));
			...
			if ($message == "") {
				...
				// Update lms_users table
				$query = "UPDATE lms_users SET username = ?, password = ?, disablelogin = ? WHERE referenceid = ? AND type = 'Sub Admin'";
				runmysqlquery($query, 'ssis', $form_username, $form_password, $disablelogin, $form_recid);

				// Update lms_subadmins table
				$query = "UPDATE lms_subadmins SET sadname = ?, sademailid = ?, transferuploadedleads = ?, cell = ?, showmcacompanies = ? WHERE id = ?";
				runmysqlquery($query, 'sssssi', $form_name, $form_email, $transferuploadedleads, $cell, $showmcacompanies, $form_recid);

				// Insert into lms_logs_event table
				$query = "INSERT INTO lms_logs_event (userid, system, eventtype, eventdatetime) VALUES (?, ?, '4', ?)";
				runmysqlquery($query, 'sss', $userslno, $_SERVER['REMOTE_ADDR'], datetimelocal("Y-m-d H:i:s"));
	...
	case "delete":
		...
		// Delete from lms_subadmins
		$query = "DELETE FROM lms_subadmins WHERE id = ?";
		$result = runmysqlquery($query, 'i', $form_recid);

		// Delete from lms_users
		$query = "DELETE FROM lms_users WHERE referenceid = ? AND type = 'Sub Admin'";
		$result = runmysqlquery($query, 'i', $form_recid);

		$query = "INSERT INTO lms_logs_event (userid, system, eventtype, eventdatetime) VALUES (?, ?, ?, ?)";
		$result = runmysqlquery($query, 'ssss', $userslno, $remoteAddr, '5', $eventDateTime);
	...
	case "gridtoform":
		...
		$query1 = "SELECT * FROM lms_users WHERE referenceid = ? AND type = 'Sub Admin'";
		$query2 = "SELECT * FROM lms_subadmins WHERE id = ?";

		// Execute the first query with parameter binding
		$result1 = runmysqlqueryfetch($query1, 'i', $form_recid);

		// Execute the second query with parameter binding
		$result2 = runmysqlqueryfetch($query2, 'i', $form_recid);
