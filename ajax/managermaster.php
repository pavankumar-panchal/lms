...
switch ($submittype) {
	case "save":
		...
		if ($form_recid == "") {
			...
			if ($message == "") {
				...
				$query = "SELECT * FROM lms_managers WHERE mgrname = ?";
			...
			if ($message == "") {
				...
				// First query to insert into lms_managers
				$query = "INSERT INTO `lms_managers` (mgrname, mgrlocation, mgremailid, mgrcell, transferuploadedleads, managedarea, branchhead, branch, showmcacompanies) 
                  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
				$result = runmysqlquery($query, 'sssssssss', $form_name, $form_location, $form_email, $form_cell, $transferuploadedleads, $managedarea, $branchhead, $branch, $showmcacompanies);

				// Query to select the id from lms_managers
				$query = "SELECT id FROM lms_managers WHERE mgrname = ?";
				$result = runmysqlqueryfetch($query, 's', $form_name);

				// Inserting into lms_users
				$query = "INSERT INTO `lms_users` (username, password, type, referenceid, lastlogindate, logincount, disablelogin) 
                 VALUES (?, ?, 'Reporting Authority', ?, '', '0', ?)";
				$result = runmysqlquery($query, 'sssi', $form_username, $form_password, $result['id'], $disablelogin);
			...
			if ($message == "") {
				...
				$query1 = "UPDATE lms_users SET username = ?, password = ?, disablelogin = ? WHERE referenceid = ? AND type = 'Reporting Authority'";
				$result1 = runmysqlquery($query1, 'ssss', $form_username, $form_password, $disablelogin, $form_recid);

				$query2 = "UPDATE lms_managers SET mgrname = ?, mgrlocation = ?, mgremailid = ?, mgrcell = ?, transferuploadedleads = ?, managedarea = ?, branch = ?, branchhead = ?, showmcacompanies = ? WHERE id = ?";
				$result2 = runmysqlquery($query2, 'ssssssssss', $form_name, $form_location, $form_email, $form_cell, $transferuploadedleads, $managedarea, $branch, $branchhead, $showmcacompanies, $form_recid);
	...
	case "delete":
		...
		if ($message == "") {
			...
			$query = "SELECT * FROM dealers WHERE managerid = ?";
			$result = runmysqlquery($query, 's', $form_recid);
		...
		if ($message == "") {
			...
			// First DELETE query
			$query1 = "DELETE FROM lms_managers WHERE id = ?";
			$result1 = runmysqlquery($query1, 's', $form_recid);

			// Second DELETE query
			$query2 = "DELETE FROM lms_users WHERE referenceid = ? AND type = 'Reporting Authority'";
			$result2 = runmysqlquery($query2, 's', $form_recid);
	...
	case "gridtoform":
		...
		$query1 = "SELECT * FROM lms_users WHERE referenceid = ? AND type = ?";
		$result1 = runmysqlqueryfetch($query1, 'is', $form_recid, 'Reporting Authority');

		$query2 = "SELECT * FROM lms_managers WHERE id = ?";
