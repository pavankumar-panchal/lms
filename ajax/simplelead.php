...
switch ($submittype) {
	...
	case "quote_generate":
		...
		$product_name = ($_POST['productnames']);
		$purchase_type = ($_POST['purchasetypes']);
		$usage_type = ($_POST['usagetypes']);
...
where  " . $datepiece . " and lms_users.username = '" . $cookie_username . "' and lms_users.type = 'Dealer Member'";
	...
	// 	// Assuming $leadid is already defined in your code
	// 	$leadid = $_POST['id'];
	// 	echo $leadid;

	// 	// Fetch details from leads table
	// 	$query = "SELECT * FROM leads WHERE id = '" . $leadid . "'";
	// 	$fetch = runmysqlqueryfetch($query);
	...
	// 	// Set quotenumber to the specified format (RSL + current year + autoincrement value)
	// 	$year = date("Y");
	// 	$quoteNumberQuery = "SELECT MAX(id) as max_id FROM lms_quote";
	// 	$quoteNumberResult = runmysqlqueryfetch($quoteNumberQuery);
	// 	$autoIncrementValue = $quoteNumberResult['max_id'] + 1;
	// 	$quotenumber = "RSL" . $year . str_pad($autoIncrementValue, 2, "0", STR_PAD_LEFT);

	// 	// Set validdate to next 14 days from the current date
	// 	$validdate = date("Y-m-d", strtotime("+14 days"));
	...
	// 	$customername = $fetch['name'];
	// 	$cell = $fetch['cell'];
	// 	$extvname = $contactperson; // Assuming $contactperson is defined
	// 	$extvcell = $cell;
	// 	$extvemailid = $emailid;
	// 	$gstin = $_POST['gstin']; // Assuming gstin is received from the form

	// 	// SQL query to insert into lms_quote
	// 	$insertQuery = "INSERT INTO lms_quote (
	// 		`quotedate`,
	// 		`quotenumber`,
	// 		`validdate`,
	// 		`leadid`,
	...
	// 	$result = runmysqlquery($insertQuery);
